...
def get_positions_draw(username, url_list):
    ...
    for index in range(2):#range(len(url_list)):
        ...
        for event in telemetry_json:
            ...
            if event["_T"] in ("LogPlayerPosition", "LogParachuteLanding"):
                if event["character"]["name"] == username:
                        if event['common']['isGame'] > 0 and  event['common']['isGame'] <= 0.15:
        ...
        map_road = plt.figure(figsize=(9, 9))
        map_img = mpld3.fig_to_html(map_road)
...
def main():
    ...
    player_match_ids = get_player_data(username, api_key)
